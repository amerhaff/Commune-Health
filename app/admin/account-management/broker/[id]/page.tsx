...
export default function EmployeeRosterPage({
  ...
  const [commissionData, setCommissionData] = useState({ annualCommission: 241800, ytdCommission: 201500, monthlyCommission: 22650, totalEnrollees: 221, commissionGrowth: 15.5, enrolleeGrowth: 8.2 }) // Added commissionData
  ...
  useEffect(() => {
    ...
    figures.employerClients = employers.length; // Correct calculation for employerClients
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Brokerage Name</h1>
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      ...
      <Card className="relative">
        <div className="absolute inset-x-0 top-0 h-2 bg-[#1400FF] rounded-t-md"></div>
        <CardHeader>
          <CardTitle>More Brokerage Information</CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="enrollee-list" className="w-full mb-4" onValueChange={setActiveSubTab}>
            <TabsList>
              <TabsTrigger value="enrollee-list" className={cn("data-[state=active]:bg-black data-[state=active]:text-white")}>Client List</TabsTrigger>
              <TabsTrigger value="revenues" className={cn("data-[state=active]:bg-black data-[state=active]:text-white")}>Comissions</TabsTrigger>
            ...
            <TabsContent value="enrollee-list">
              ...
              <div className="space-y-4">
                {employers.map((employer) => (
                  <Card key={employer.id} className="relative pt-0"> {/* Removed pt-6 class, added relative */}
                  <div className="absolute inset-x-0 top-0 h-2 bg-[#1400FF] rounded-t-md"></div>
                   <div className="p-6 pl-12"> {/* Added pl-12 for left indent */}
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                ...
                                <TableHead>Company Name</TableHead><TableHead>Company Address</TableHead>
                                <TableHead>Company Phone</TableHead>
                                <TableHead>Actions</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {employer.employees.map((employee) => (                                <React.Fragment key={employee.id}>                                  <TableRow>
                                    <TableCell>{employee.dependents.length > 0 &&<Button variant="ghost" size="sm" className="p-0" onClick={() => toggleEmployeeExpansion(employee.id)}>{expandedEmployeeIds.includes(employee.id) ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</Button>}</TableCell>
                                    ...
                                    <TableCell>
                                      {employee.companyAddress && `${employee.companyAddress.street}, ${employee.companyAddress.city}, ${employee.companyAddress.state} ${employee.companyAddress.zipCode}`}
                                    </TableCell>
                                    <TableCell>{employee.companyPhone || "N/A"}</TableCell>
                                    <TableCell>
                                  ...
                                  {expandedEmployeeIds.includes(employee.id) &&
                                    employee.dependents.map((dependent) => (
                                      <TableRow key={dependent.id} className="bg-gray-100">
                                        ...
                                        <TableCell>{dependent.dateOfBirth || "N/A"}</TableCell>
                                        <TableCell>{dependent.sex || "N/A"}</TableCell>
                                        ...
                                        <TableCell>{dependent.dpcProvider || "N/A"}</TableCell>
                                        <TableCell>{dependent.companyName || "N/A"}</TableCell>
                                        <TableCell>
                                          {dependent.companyAddress && `${dependent.companyAddress.street}, ${dependent.companyAddress.city}, ${dependent.companyAddress.state} ${dependent.companyAddress.zipCode}`}
                                        </TableCell>
                                        <TableCell>{dependent.companyPhone || "N/A"}</TableCell>
                                        <TableCell>
            ...
            <TabsContent value="revenues" className="pt-0"> {/* Removed default top padding */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <Card className="relative pt-6"> {/* Added pt-6 class */}
                  <div className="absolute inset-x-0 top-0 h-2 bg-[#1400FF] rounded-t-md"></div>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium z-10">Annual Commission</CardTitle>
                    <DollarSign className="h-4 w-4 text-blue-600 z-10" /> {/* Blue icon */}
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">${commissionData.annualCommission.toLocaleString()}</div>
                    <p className="text-xs text-muted-foreground">
                      <span className="text-green-600 flex items-center">
                        <ArrowUpRight className="mr-1 h-4 w-4 text-blue-600" /> {/* Blue icon */}
                        {commissionData.commissionGrowth}% from last year
                      </span>
                    </p>
                  </CardContent>
                </Card>
                <Card className="relative pt-6"> {/* Added pt-6 class */}
                  <div className="absolute inset-x-0 top-0 h-2 bg-[#1400FF] rounded-t-md"></div>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium z-10">Year-to-Date Commission</CardTitle>
                    <Calendar className="h-4 w-4 text-blue-600 z-10" /> {/* Blue icon */}
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">${commissionData.ytdCommission.toLocaleString()}</div>
                ...
                <Card className="relative pt-6"> {/* Added pt-6 class */}
                  <div className="absolute inset-x-0 top-0 h-2 bg-[#1400FF] rounded-t-md"></div>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium z-10">Monthly Commission</CardTitle>
                ...
                <Card className="relative pt-6"> {/* Added pt-6 class */}
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      <span className="text-green-600 flex items-center">
                        <ArrowUpRight className="mr-1 h-4 w-4 text-blue-600" /> {/* Blue icon */}
                        {commissionData.enrolleeGrowth}% from last year
                      </span>
                    </p>
                  </CardContent>
                </Card>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Enrollee Name</TableHead> {/* Updated */}
                        <TableHead>Employer Name</TableHead> {/* Updated */}
                        <TableHead>DPC Provider</TableHead> {/* Updated */}
                    ...
                    <TableBody>
                      {transactions.map((transaction) => (
                        <TableRow key={transaction.id}>
                          ...
                          <TableCell className="border-b border-gray-200 px-4 py-2">{transaction.enrolleeName}</TableCell> {/* Updated */}
                          <TableCell className="border-b border-gray-200 px-4 py-2">{transaction.employerName}</TableCell> {/* Updated */}
                          <TableCell className="border-b border-gray-200 px-4 py-2">{transaction.broker}</TableCell> {/* Updated */}
